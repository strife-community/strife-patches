...
-- This is for the filter headers
local filterActivatable 	= false
	
local filterPower			= false
local filterAttackDamage 	= false
local filterAttackSpeed		= false
local filterAttackMod		= false

local filterArmor			= false
local filterMagicArmor		= false
local filterMitigation		= false
local filterResistance		= false
local filterHealth			= false
local filterLifeSteal		= false

local filterMana			= false
local filterCooldown		= false

local filterDebuffEnemy		= false
local filterMobility		= false
local filterCrit			= false
local filterBuff			= false

local filterHealthComp	 	= false
local filterHealthRegenComp	= false
local filterManaComp	 	= false
local filterManaRegenComp 	= false
local filterPowerComp		= false
local filterAttackSpeedComp	= false
...
local filterTable = {
	...
	health_regen_comp 	= {'health_regen_comp'},
	mana_comp 			= {'mana_comp'},
	mana_regen_comp 	= {'mana_regen_comp'},
	power_comp 			= {'power_comp'},
	attack_speed_comp 	= {'attack_speed_comp'},
...
local filterVisibilityTable = {
	...
	components = {
		health_comp			= true,
		health_regen_comp	= true,
		mana_comp			= true,
		mana_regen_comp		= true,
		power_comp			= true,
		attack_speed_comp	= true,
	},
}
...
local function ActivateRelatedFilters(filter)
	...
	if trigger_shopFilter.forceCategory == '' then
		...
		if (trigger_shopFilter.shopCategory) and (not Empty(trigger_shopFilter.shopCategory)) and ((not ((isFiltering))) or (trigger_gamePanelInfo.shopShowFilters)) then
...
local function SetFilterHeader (filter, isActive)	
	if filter == 'activatable' then
		filterActivatable 		= isActive
	
	elseif filter == 'ability_power' then
		filterPower 			= isActive
	
	elseif filter == 'attack_damage' then
		filterAttackDamage 		= isActive
	
	elseif filter == 'attack_speed' then
		filterAttackSpeed 		= isActive
	
	elseif filter == 'attack_mod' then
		filterAttackMod 		= isActive
	
	elseif filter == 'defense_armor' then
		filterArmor 			= isActive
	
	elseif filter == 'defense_magic_armor' then
		filterMagicArmor 		= isActive
	
	elseif filter == 'defense_mitagiton' then
		filterMitigation		= isActive
		
	elseif filter == 'defense_resistance' then
		filterResistance		= isActive
		
	elseif filter == 'defense_health' then
		filterHealth 			= isActive
	
	elseif filter == 'attack_lifesteal' then
		filterLifeSteal 		= isActive
	
	elseif filter == 'ability_mana' then
		filterMana 				= isActive
	
	elseif filter == 'cd_reduction' then
		filterCooldown 			= isActive
	
	elseif filter == 'debuff_enemy' then
		filterDebuffEnemy 		= isActive
		
	elseif filter == 'mobility' then
		filterMobility 			= isActive
		
	elseif filter == 'attack_crit' then
		filterCrit 				= isActive
		
	elseif filter == 'health_comp' then
		filterHealthComp		= isActive
		
	elseif filter == 'health_regen_comp' then
		filterHealthRegenComp	= isActive
	
	elseif filter == 'mana_comp' then
		filterManaComp			= isActive
	
	elseif filter == 'mana_regen_comp' then
		filterManaRegenComp		= isActive
		
	elseif filter == 'power_comp' then
		filterPowerComp			= isActive
	
	elseif filter == 'attack_speed_comp' then
		filterAttackSpeedComp	= isActive
...
local function gameShopRegisterFilterCheckbox(object, filter)
	...
	local frame			= object:GetWidget('shopFilterCheckbox'..filter..'_frame')
	local name			= object:GetWidget('shopFilterCheckbox'..filter..'_name')
	local checkButton	= object:GetWidget('shopFilterCheckbox'..filter..'CheckButton')
	local checkFaint	= object:GetWidget('shopFilterCheckbox'..filter..'CheckFaint')
	local check			= object:GetWidget('shopFilterCheckbox'..filter..'Check')
	
	local currentNameColor		= '#ffffff'
	local currentFrameColor		= '#226479'

	if (not button) then return end

	button:SetCallback('onmouseover', function(widget)
		frame:SetColor('#218aab')
		frame:SetBorderColor('#218aab')
		name:SetColor('#00d7ff')
		UpdateCursor(widget, true, { canLeftClick = true})
	end)

	button:SetCallback('onmouseout', function(widget)
		frame:SetColor(currentFrameColor)
		frame:SetBorderColor(currentFrameColor)
		name:SetColor(currentNameColor)
		UpdateCursor(widget, false, { canLeftClick = true})
	end)
	...
	checkButton:SetCallback('onmouseover', function(widget)
		checkFaint:FadeIn(200)
		checkFaint:SetColor(1, 1, 1, 0.3)
		frame:SetColor('#218aab')
		frame:SetBorderColor('#218aab')
		name:SetColor('#00d7ff')
		UpdateCursor(widget, true, { canLeftClick = true})
	...
	checkButton:SetCallback('onmouseout', function(widget)		
		checkFaint:FadeOut(150)
		checkFaint:SetColor(1, 1, 1, 0)
	...
	check:RegisterWatchLua('gameShopFilterInfo', function(widget, trigger)
		...
		if trigger[filter] then
			currentFrameColor 	= '#226479'
			currentNameColor 	= '#ffffff'
		else
			currentFrameColor	= '#114657'
			currentNameColor	= '#7a93a0'
		end
...
local function gameShopRegisterItemList(object)
	...
	local scrollMax					= 0
	local scrollPos					= 0
	local itemsPerRow				= 8	-- Will change based on whether filters are visible
	local itemsPerRowMax			= 8

	local itemPaddingBase			= libGeneral.HtoP(0.8)
	local itemPaddingList			= itemPaddingBase
	local itemPaddingSimple			= itemPaddingBase
	local itemHeightSimpleBase		= libGeneral.HtoP(7.65)
	local itemHeightListBase		= libGeneral.HtoP(8.5)
	...
	local function gameShopUpdateGlowColor(innerGlow, outerFrame, shoppingIcon, over, isOnCourier, isInStash, isOwned, canAfford, isRecipeScroll, itemCanAfford, needComponents)
		...
		if innerGlow and outerFrame and shoppingIcon then
			...
			if isOwned then
				showIcon	= true
				if isInStash then
					iconTexture = '/ui/new_game/shop/textures/shop_hoverIcon_stash.tga'
					updateTexture = true
					glowColor	= '#efd05635'
					frameColor	= '#efd056'
				elseif isOnCourier then
					iconTexture = '/ui/new_game/shop/textures/shop_hoverIcon_courier.tga'
					updateTexture = true
					glowColor	= '#efd05635'
					frameColor	= '#efd056'
				else
					iconTexture = '/ui/new_game/shop/textures/shop_hoverIcon_owned.tga'
					updateTexture = true
					glowColor	= '#efd05635'
					frameColor	= '#efd056'
				end
			...
			if over then
				...
				--outerFrame:SetColor('#074551')
				outerFrame:SetBorderColor('0 0 0 0.4')
			end
		else
	...
	local function updateRowHeight(object)
		...
		if trigger_gamePanelInfo.shopItemView == 0 then
			...
			local itemPaddingSimpleAdd		= floor(viewAreaSimple / (maxItemsSimple - 1)) + 10
			itemPaddingSimple				= itemPaddingBase + itemPaddingSimpleAdd
			itemHeightSimpleExpanded		= (itemHeightSimple * 2) + itemPaddingSimple
	...
	listContainer:RegisterWatchLua('shopVisControls', function(widget, groupTrigger)
		...
		if showFilters and (string.len(trigger_shopFilter.forceCategory) <= 0) then
			widget:SetWidth(libGeneral.HtoP(-16.5))
			widget:SetX(0)
		else
			widget:SetWidth('100%')
			widget:SetX(0)
	...
	local function buttonRegisterActions(button, index, icon, componentID, recipeScroll, innerGlow, outerFrame, shoppingIcon, bg, hover)	-- rmm update to support component dragging		
		local paramPrefix = ''
	...
	local function updateItemCostStatus(widget, trigger, paramPrefix, isComponent, componentPrefix, iconImage)
		...
		if trigger[paramPrefix..'isOwned'] then
			...
			widget:SetColor('#12ff00')
			widget:SetOutlineColor('#03240085')
			
		elseif trigger[paramPrefix..'canAfford'] or shopInLauncher then
			widget:SetText(libNumber.commaFormat(costValue))
			if isLowered then
				widget:SetColor('#ffb320')
				widget:SetOutlineColor('#29150085')
			else
				widget:SetColor('#ffb320')
				widget:SetOutlineColor('#29150085')
			end
			...
			widget:SetText(libNumber.commaFormat(costValue))
			widget:SetColor('#e02828')
			widget:SetOutlineColor('#24000085')
	...
	local function registerItemListEntryDetailed(object, rowID, itemID)
		...
		local listHover				= object:GetWidget('gameShopItemListItem'..rowID..'ListHover')
		local listDarkBG			= object:GetWidget('gameShopItemListItem'..rowID..'ListDarkBG')
		local listBacker			= object:GetWidget('gameShopItemListItem'..rowID..'ListBacker')
		
		listDarkBG:SetVisible(infoTrigger.isOwned)
		...
		end, false, fullItemWatchList, 'icon', 'isOwned', 'canAfford')
		...
		local function costIconContainerUpdate(widget, trigger)
		...
		costIconContainer:RegisterWatchLua('ShopItem'..itemID, function(widget, trigger)
			costIconContainerUpdate(widget, trigger)
		end, true, fullItemWatchList, 'isOwned', 'isRecipe')
		costIconContainerUpdate(costIconContainer, infoTrigger)
		...
		listDarkBG:UnregisterWatchLua('ShopItem'..itemID)
		listDarkBG:RegisterWatchLua('ShopItem'..itemID, function(widget, trigger)
			widget:SetVisible(trigger.isOwned)
		end, false, fullItemWatchList, 'isOwned')

		buttonRegisterActions(button, itemID, icon, nil, nil, innerGlow, outerFrame, shoppingIcon, listBG, listHover)
	end
	...
	local function registerItemListEntrySimple(object, prefix, rowID, subID, itemID, isComponent)
		...
		local background			= object:GetWidget(prefix..rowID..'Item'..subID..'BG')
		local dark					= object:GetWidget(prefix..rowID..'Item'..subID..'DarkBG')
		local hover					= object:GetWidget(prefix..rowID..'Item'..subID..'Hover')
		...
		if isComponent then
			...
			buttonRegisterActions(button, itemID, icon, (subID - 1), nil, innerGlow, outerFrame, shoppingIcon, background, hover)
			...
			costIconContainer:RegisterWatchLua('ShopItem'..itemID, function(widget, trigger)
				costIconContainerUpdate(widget, trigger)
			end, true, fullItemWatchSimple, 'isOwned', 'isRecipe')
			costIconContainerUpdate(costIconContainer, infoTrigger)
			...
			buttonRegisterActions(button, itemID, icon, nil, nil, innerGlow, outerFrame, shoppingIcon, background, hover)
			...
			dark:UnregisterWatchLua('ShopItem'..itemID)
			dark:RegisterWatchLua('ShopItem'..itemID, function(widget, trigger)
				widget:SetVisible(trigger.isOwned)
			end, false, fullItemWatchSimple, 'isOwned')
			
			if infoTrigger[paramPrefix..paramExists] then
				dark:SetVisible(infoTrigger[paramPrefix..paramOwned])
			end
		end
	...
	function ShopUI.openItemSwapPopup(openedID, rowID, subID) -- Needs to be added to a table for visibility
		...
		for i = 1, infoTable.num do
			...
			container:AddTemplateListItem(
				'gameShopItemSwapEntry', i,
	...
	local function registerSelectedItemSub(object, rowID, selectedItem)
		...
		if trigger_gamePanelInfo.shopItemView == 0 then
			...
			selectedBackerSimple:ScaleHeight(libGeneral.HtoP(8.8), 175)
			selectedBackerSimple:FadeIn(175)
			selectedBackerList:SetVisible(false)
			selectedContainer:SetY(libGeneral.HtoP(9.6))
			selectedContainer:SetHeight(libGeneral.HtoP(6.5))
		else
			...
			selectedContainer:SetY(libGeneral.HtoP(10.4))
			selectedContainer:SetHeight(libGeneral.HtoP(6))
		end
		...
		local function updateScrollCost(widget, trigger)
			...
			if trigger.isOwned then
				if trigger.isRecipe then
					widget:SetText(Translate('general_owned'))
					widget:SetColor('#828282')
					widget:SetOutlineColor('#1d1d1d85')
				else
					widget:SetText(libNumber.commaFormat(trigger.recipeScrollCost))
					widget:SetColor('#ffa800')
					widget:SetOutlineColor('#26130085')
				end
			...
			elseif (not trigger.canAfford) then
				...
				widget:SetText(libNumber.commaFormat(trigger.recipeScrollCost))
				widget:SetColor('#ffb320')
				widget:SetOutlineColor('#32190090')
			end
		end
	...
	local function updateRowDataSelectedID(object)
		for rowID, rowContents in ipairs(rowData) do
			...
			if ID == selected then
				...
				if (listBacker) then listBacker:FadeIn(125) end
			...
			elseif rowContents.subList and type(rowContents.subList) == 'table' and #rowContents.subList > 0 then
				...
				for subIndex, subItemID in ipairs(rowContents.subList) do
					if subItemID == selected then
						if object:GetWidget('gameShopItemListRow'..rowID..'Item'..subIndex..'DarkBG'):IsVisible() then
							lastSelectedRow = rowID
							lastSelectedIndex = subIndex
						end
					
						-- Expand swap icon, move the label etc
						object:GetWidget('gameShopItemListRow'..rowID..'Item'..subIndex..'SwapClickLabel'):FadeIn(125)	
						if (listBacker) then listBacker:FadeOut(125) end
						object:GetWidget('gameShopItemListRow'..rowID..'Item'..subIndex..'DarkBG'):SetVisible(false)
						...
						if lastSelectedRow ~= -1 and lastSelectedIndex ~= -1 and lastSelectedRow == rowID and lastSelectedIndex == subIndex then
							object:GetWidget('gameShopItemListRow'..rowID..'Item'..subIndex..'DarkBG'):SetVisible(1)
						end
	...
	local function buildRowData(object, itemList)	-- Build the data structure for populateList
		...
		local floatPadding = libGeneral.HtoP(1.5)
		if filtersVisible then
			itemsPerRow = 5
			floatPadding = libGeneral.HtoP(1.5)
		else
			itemsPerRow = itemsPerRowMax
		end
	...
	libScrollShop2.register(
		...
		{
			scrollbar		= scrollBar,
			scrollbarPlaceholder = object:GetWidget('gameShopItemListScrollbarBlank'),
	...
	local function TransferItemBuildToBookmarks()
		...
		if (selectedGuideTable) then
			for i,v in pairs(selectedGuideTable) do
				if ValidateEntity(v) and v ~= "Item_Purchasable_XP" then
...
local function gameShopRegister(object)
	...
	local itemViewButtonSimpleFrame			= object:GetWidget('gameShopItemViewButtonSimpleFrame')
	local itemViewButtonSimpleIcon			= object:GetWidget('gameShopItemViewButtonSimpleIcon')
	local itemViewButtonSimpleHover			= object:GetWidget('gameShopItemViewButtonSimpleHover')
	...
	local itemViewButtonDetailedFrame		= object:GetWidget('gameShopItemViewButtonDetailedFrame')
	local itemViewButtonDetailedIcon		= object:GetWidget('gameShopItemViewButtonDetailedIcon')
	local itemViewButtonDetailedHover		= object:GetWidget('gameShopItemViewButtonDetailedHover')
	...
	local header01		= object:GetWidget('shopFilter_general')
	local header02		= object:GetWidget('shopFilter_damage')
	local header03		= object:GetWidget('shopFilter_defense')
	local header04		= object:GetWidget('shopFilter_mana')
	local header05		= object:GetWidget('shopFilter_utility')
	local header06		= object:GetWidget('shopFilter_components')
	local header07		= object:GetWidget('shopFilter_blank')
	...
	itemViewButtonSimple:SetCallback('onmouseover', function(widget)
		itemViewButtonSimpleHover:FadeIn(200)
	...
	itemViewButtonSimple:SetCallback('onmouseout', function(widget)
		itemViewButtonSimpleHover:FadeOut(100)
		simpleTipGrowYUpdate(false)
	end)

	itemViewButtonDetailed:SetCallback('onmouseover', function(widget)
		itemViewButtonDetailedHover:FadeIn(200)
	...
	itemViewButtonDetailed:SetCallback('onmouseout', function(widget)
		itemViewButtonDetailedHover:FadeOut(100)
		simpleTipGrowYUpdate(false)
	end)
	...
	itemViewButtonSimpleBacker:RegisterWatchLua('gamePanelInfo', function(widget, trigger)
		if trigger.shopItemView == 0 then
			widget:SetColor(1, 1, 1, 1)
			itemViewButtonSimpleFrame:SetColor(1, 1, 1, 1)
			itemViewButtonSimpleIcon:SetColor('#a4edff')
		else
			widget:SetColor(0.8, 0.8, 0.8, 1)
			itemViewButtonSimpleFrame:SetColor(0.8, 0.8, 0.8, 1)
			itemViewButtonSimpleIcon:SetColor('#5dadc1')
		end
	...
	itemViewButtonDetailedBacker:RegisterWatchLua('gamePanelInfo', function(widget, trigger)
		if trigger.shopItemView == 1 then
			widget:SetColor(1, 1, 1, 1)
			itemViewButtonDetailedFrame:SetColor(1, 1, 1, 1)
			itemViewButtonDetailedIcon:SetColor('#a4edff')
		else
			widget:SetColor(0.8, 0.8, 0.8, 1)
			itemViewButtonDetailedFrame:SetColor(0.8, 0.8, 0.8, 1)
			itemViewButtonDetailedIcon:SetColor('#5dadc1')
		end
	end, false, nil, 'shopItemView')
	...
	container:RegisterWatchLua('shopVisControls', function(widget, groupTrigger)
		...
		elseif gamePhase >= 4 then
			...
			if trigger_gamePanelInfo.abilityPanel then
				local levelUpContainer	= object:GetWidget('levelUpPanel')
				
				widget:SetHeight(levelUpContainer:GetHeight() + 10, 150)
			else
				widget:SetHeight('-20h', 150)
			end
	...
	local function displayRelevantFilters(sourceWidget, category)
		...
		for filterIndex,filterValueTable in pairs(filterTable) do
			if (filterVisibilityTable[category]) and (filterVisibilityTable[category][filterIndex]) then
				...
				SetFilterHeader(filterIndex, true)
				trigger_gamePanelInfo.shopHasFiltersToDisplay = true
			else
				sourceWidget:GetWidget('shopFilterCheckbox'..filterIndex):SetVisible(0)
				SetFilterHeader(filterIndex, false)
			end
			
			header01:SetVisible(filterActivatable)
			header02:SetVisible(filterPower or filterAttackDamage or filterAttackSpeed or filterAttackMod)
			header03:SetVisible(filterArmor or filterMagicArmor or filterHealth or filterLifeSteal or filterMitigation or filterResistance)
			header04:SetVisible(filterMana or filterCooldown)
			header05:SetVisible(filterDebuffEnemy or filterMobility or filterCrit or filterBuff)
			header06:SetVisible(filterHealthComp or filterHealthRegenComp or filterManaComp or filterManaRegenComp or filterPowerComp or filterAttackSpeedComp)
			header07:SetVisible(not filterActivatable and not filterPower and not filterAttackDamage and not filterAttackSpeed and not filterAttackMod and not filterArmor and not filterMagicArmor and not filterHealth and not filterLifeSteal and not filterMitigation and not filterResistance and not filterMana and not filterCooldown and not filterDebuffEnemy and not filterMobility and not filterCrit and not filterBuff and not filterHealthComp and not filterHealthRegenComp and not filterManaComp and not filterManaRegenComp and not filterPowerComp and not filterAttackSpeedComp)
		end
	end

	object:GetWidget('gameShopFilterList'):RegisterWatchLua('gamePanelInfo', function(widget, trigger)
		local itemContainer 	= widget:GetWidget('gameShopItemListBody')
		...
		if (trigger.shopView == 1 and not trigger.abilityPanel) then
			if ((trigger.shopShowFilters) or (trigger.shopCategory == '') or (trigger.shopCategory == 'components')) then
				...
				if (trigger.shopHasFiltersToDisplay) and string.len(trigger_shopFilter.forceCategory) <= 0 then
					widget:SetVisible(1)
					itemContainer:SetX('0.6h')
				else
					widget:SetVisible(0)
					itemContainer:SetX('-0.1h')
				end
			else
				widget:SetVisible(0)
				itemContainer:SetX('-0.1h')
	...
	object:GetWidget('shopToggleFilterBtn'):SetCallback('onclick', function(widget)
		...
		trigger_gamePanelInfo:Trigger(false)
		LuaTrigger.GetTrigger('shopItemListParams'):Trigger(true)
	end)

	object:GetWidget('shopToggleFilterBtn'):SetCallback('onmouseover', function(widget)
		object:GetWidget('shopToggleFilterBtnHover'):FadeIn(200)
	...
	object:GetWidget('shopToggleFilterBtn'):SetCallback('onmouseout', function(widget)
		object:GetWidget('shopToggleFilterBtnHover'):FadeOut(100)
		simpleTipGrowYUpdate(false)
	end)
	...
	object:GetWidget('shopToggleFilterBtnBacker'):RegisterWatchLua('gamePanelInfo', function(widget, trigger)
		if (trigger.shopShowFilters) then
			widget:SetColor(1, 1, 1, 1)
			widget:GetWidget('shopToggleFilterBtnFrame'):SetColor(1, 1, 1, 1)
			widget:GetWidget('shopToggleFilterBtnIcon'):SetColor('#a4edff')
		else
			widget:SetColor(0.8, 0.8, 0.8, 1)
			widget:GetWidget('shopToggleFilterBtnFrame'):SetColor(0.8, 0.8, 0.8, 1)
			widget:GetWidget('shopToggleFilterBtnIcon'):SetColor('#5dadc1')
		end
	...
	-- there is already a register for the courier button in inventory.lua
...
local trigger_shopItemTipIntermediary_paramList = {
	...
	{ name	= 'recipeComponentDetail'..(0)..'damage',					type		= 'number' },
	...
	{ name	= 'recipeComponentDetail'..(1)..'damage',					type		= 'number' },
	...
	{ name	= 'recipeComponentDetail'..(2)..'damage',					type		= 'number' },
	...
	{ name	= 'recipeComponentDetail'..(3)..'damage',					type		= 'number' },
...
local function shopItemTipRegisterStat(object, statName, heroTriggerName, iconPath)
	local container				= object:GetWidget('shopItemTipStat_' .. statName)
	local name					= object:GetWidget('shopItemTipStat_' .. statName .. '_name')
	local current				= object:GetWidget('shopItemTipStat_' .. statName .. '_current')
	local item					= object:GetWidget('shopItemTipStat_' .. statName .. '_item')
	local change				= object:GetWidget('shopItemTipStat_' .. statName .. '_change')

	local widgetAD				= object:GetWidget('shopItemTipStat_baseAttackDamage')
	local widgetAP				= object:GetWidget('shopItemTipStat_baseAbilityDamage')
	local nameAD				= object:GetWidget('shopItemTipStat_baseAttackDamage_name')
	local nameAP				= object:GetWidget('shopItemTipStat_baseAbilityDamage_name')
	local currentAD				= object:GetWidget('shopItemTipStat_baseAttackDamage_current')
	local currentAP				= object:GetWidget('shopItemTipStat_baseAbilityDamage_current')
	local itemAD				= object:GetWidget('shopItemTipStat_baseAttackDamage_item')
	local itemAP				= object:GetWidget('shopItemTipStat_baseAbilityDamage_item')
	local changeAD				= object:GetWidget('shopItemTipStat_baseAttackDamage_change')
	local changeAP				= object:GetWidget('shopItemTipStat_baseAbilityDamage_change')
		

	local statTriggerParams = {
	...
	libGeneral.createGroupTrigger('shopItemTipStatGroup'..heroTriggerName, statTriggerParams)
	...
	container:RegisterWatchLua('shopItemTipStatGroup'..heroTriggerName, function(widget, groupTrigger)		
		...
		if (triggerItem.isOwned or true) then
		...
		if (ownedValue > 0) then				
			...
			local value1			= itemStatTypeFormat_itemTip[statName](heroValue + fullItemValue - ownedValue)
			local value2			= itemStatTypeFormat_itemTip[statName](heroValue + fullItemValue)
			local value3			= itemStatTypeFormat_itemTip[statName](fullItemValue)
			
			local triggerSelf		= LuaTrigger.GetTrigger('AltInfoSelf')
			local totalAttackDamage	= triggerSelf.damage
			
			local basePower			= triggerSelf.power / 100
			local itemDamage 		= triggerItem.damage
	
			if triggerItem.isRecipe then
				for i=0,2,1 do
					if triggerItem['recipeComponentDetail'..i..'exists'] and (triggerItem['recipeComponentDetail'..i..'isOwned'] or true) then
						itemDamage = itemDamage + triggerItem['recipeComponentDetail'..i..'damage']
					end
				end
			end
			

			local newAttackDamage 	= 0
			local differenceDamage 	= 0
	
			local baseAttackDamage  = totalAttackDamage / basePower -- basically damage without power
				
			if statName == 'power' then
				--AP (Ability Damage/Power) will be = Power
				widgetAP:SetVisible(1)
				
				nameAP:SetColor('#006b93')				
				currentAP:SetText(value1 .. valueSuffix)
				itemAP:SetText(value2..valueSuffix)
				changeAP:SetText('+' .. value3 .. valueSuffix .. '*')
				
				--AD (Attack Damage) needs to be calculated
				powerPercent 		= ( value3 / 100 ) + basePower				
				newAttackDamage 	= ( baseAttackDamage + itemDamage ) * powerPercent
				differenceDamage	= newAttackDamage - totalAttackDamage
			
				widgetAD:SetVisible(1)
			
				nameAD:SetColor('#006b93')		
				currentAD:SetText(libNumber.commaFormat(floor(totalAttackDamage)) .. valueSuffix)
				itemAD:SetText(libNumber.commaFormat(floor(newAttackDamage))..valueSuffix)
				changeAD:SetText('+' .. libNumber.commaFormat(floor(differenceDamage)).. valueSuffix .. '*')
				
			elseif itemDamage > 0 then
				newAttackDamage = ( baseAttackDamage + itemDamage ) * powerPercent
				differenceDamage = newAttackDamage - baseAttackDamage
			...
			elseif statName == 'baseAttackSpeed' then
				widget:SetVisible(1)
				name:SetColor('#006b93')
				
				valueSuffix = '%'
				local baseAttackSpeed = heroValue
				local bonusAttackSpeed = (heroValue + (ownedValue * 100))
				
				current:SetText(libNumber.round(baseAttackSpeed, 0) .. valueSuffix)
			...
			if statName == 'power' then
				widget:SetVisible(0)
				widgetAD:SetVisible(0)
				widgetAP:SetVisible(0)
				...
				nameAD:SetColor(styles_colors_stats_empty)
				nameAP:SetColor(styles_colors_stats_empty)
			else
				widget:SetVisible(0)
				current:SetText('')
				item:SetText('')
...
local function shopItemTipRegisterComponent(object, index)
	...
	local frame			= object:GetWidget('shopItemTipComponent'..index..'Frame')
	...
	container:RegisterWatchLua('shopItemTipIntermediary', function(widget, trigger)
		if trigger['recipeComponentDetail'..index..'exists'] then
			if not icon:IsVisibleSelf() then
				icon:SetVisible(true)
			end
			frame:SetVisible(true)
		else
			if icon:IsVisibleSelf() then
...
local function shopItemTipRegisterImbuement(object, index)
	...
	description:RegisterWatchLua('shopItemTipIntermediary', function(widget, trigger)
		if (trigger.currentEmpoweredEffectEntityName) and (not Empty(trigger.currentEmpoweredEffectEntityName)) and ValidateEntity(trigger.currentEmpoweredEffectEntityName) then
			widget:SetText((trigger.currentEmpoweredEffectDisplayName or '?Name?')..': '..(trigger.currentEmpoweredEffectDescription or '?Description?'))
		else
...
local function shopItemTipRegister(object)
	...
	local border						= object:GetWidget('shopItemTipBorder')
	...
	shopItemTipRegisterStat(object, 'resistance', 		'resistance', 	'/ui/elements:itemtype_magdefense')
	...
	local function updateCooldown(widget, trigger, cooldownParam)
		...
		if cooldown > 0 then
			widget:SetText(libNumber.round(cooldown / 1000, 1)..'s')
		end
	end
	...
	end, false, 'shopItemWatch', 'isExpired', 'isPlayerCrafted', 'isPermanent', 'days', 'monthsLeft', 'daysLeft', 'hoursLeft', 'minutesLeft', 'entity')			
	...
	name:RegisterWatchLua('shopItemTipIntermediary', function(widget, trigger)	-- rmm component check for plain green name
		if trigger.exists then
			...
			if isRare or isLegendary then
				border:SetBorderColor(libGeneral.craftedItemGetNameColor(isRare, isRare))
				widget:SetColor(libGeneral.craftedItemGetNameColor(isRare, isRare))
			else
				border:SetBorderColor('#0b5664')
				widget:SetColor('white')
			end
		end
	end, false, nil,
	...
	local function containerTriggerUpdate(widget, triggerName, trigger, isComponent, cooldownParam, costParam, recipeParam, paramPrefix, iconParam, isInventory, isFromCrafting)
		...
		if (triggerName == 'ShopItem') and (paramPrefix == '') then
			trigger_shopItemTipIntermediary.damage = trigger[paramPrefix..'damage']
		else
			trigger_shopItemTipIntermediary.damage = 0 -- RMM StashInventory does not have this info
		end
		...
		if isComponent then
			...
			if isInventory then
				...
				for i=0,2,1 do
					...
					if (triggerName == 'ShopItem') and (paramPrefix == '') then
						trigger_shopItemTipIntermediary['recipeComponentDetail'..i..'damage'] 		= trigger['recipeComponentDetail'..i..'damage']
					else
						trigger_shopItemTipIntermediary['recipeComponentDetail'..i..'damage'] 		= 0
					end					
				...
				for i=0,2,1 do
					...
					if (triggerName == 'ShopItem') and (paramPrefix == '') then
						trigger_shopItemTipIntermediary['recipeComponentDetail'..i..'damage'] 			= trigger[componentPrefixFull..'damage']
					else
						trigger_shopItemTipIntermediary['recipeComponentDetail'..i..'damage'] 			= 0
					end						
	...
	container:RegisterWatchLua('shopItemTipInfo', function(widget, trigger)
		...
		if index >= 0 and infoTrigger and (infoTrigger.exists)  then
			...
			containerTriggerUpdate(container, triggerName, infoTrigger, isComponent, cooldownParam, costParam, recipeParam, paramPrefix, 'icon', isInventory, isFromCrafting)
			container:RegisterWatchLua(triggerName..triggerIndex, function(widget, trigger)
				containerTriggerUpdate(widget, triggerName, trigger, isComponent, cooldownParam, costParam, recipeParam, paramPrefix, 'icon', isInventory, isFromCrafting)
